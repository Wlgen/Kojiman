...
void PowerUp::init(const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram)
{
	...
	sprite->setPosition(glm::vec2(float(tileMapDispl.x + posPU.x), float(tileMapDispl.y + posPU.y)));
...
{	
	...
	if (firstTime >= 500) {
		if (!rend) {
			rend = true;
			PowerUp::initSrpite();
			sprite->setPosition(glm::vec2(float(tileMapDispl.x + 10), float(tileMapDispl.y+1)));
		}
		...
		posPU.x += movX;
		posPU.y += movY;
		if ((map->collisionPlayerLeft(posPU, glm::ivec2(32, 32))) || (map->collisionPlayerRight(posPU, glm::ivec2(32, 32)))) {
			movX = -movX;
			posPU.x += movX;
		}
		if ((map->collisionPlayerUp(posPU, glm::ivec2(32, 32))) || (map->collisionPlayerDown(posPU, glm::ivec2(32, 32)))) {
			movY = -movY;
			posPU.y += movY;
		}
...
bool PowerUp::collisionWithPlayer(glm::ivec2 posUP, glm::ivec2 posPlayer) {
	...
	for (int x = x0; x <= x1; x++)
	{
		for (int j = xp; j <= xp1; j++) {
			if (x == j) {
				if ((posUP.y >= posPlayer.y - 32) && (posPlayer.y > posUP.y + 31)) {
					return true;
				}
			}
		}
